    ...
    return (
      <div
        className={cx('Playground', {
          'Playground--full-screen': this.state.fullScreen,
        })}
      >
        <ul className="Playground__tab-list">
          {this.props.children.map((state, index) => (
            <li
              className={cx('Playground__tab', {
                'Playground__tab--active': index === currentIndex,
              })}
          ...
          <li className="Playground__toggle-size">
            <button onClick={this.handleFullScreen.bind(this)}>FS</button>
          </li>
        </ul>
