...
const PropField = ({ name, value, component, path, onChange }) => {
  const change = event =>
    onChange({ name, path, type: 'CHANGE', value: event.target.value });
...
const Tree = ({ onChange, components, parent, tree, path }) => {
  const list = [];
  const singleTypeChild =
    parent &&
    parent.ui &&
    parent.ui.children &&
    parent.ui.children.allowed &&
    parent.ui.children.allowed.length === 1
      ? parent.ui.children.allowed[0]
      : false;
  ...
  const handleAddComponent = position =>
    onChange({
      type: 'INSERT',
      path: [...path, position],
      ...(singleTypeChild ? { component: singleTypeChild } : {}),
    });
  const AddButton = ({ position }) => (
    <li>
      <button
        className="AddButton"
        onClick={handleAddComponent.bind(this, position)}
      >
        + {singleTypeChild ? singleTypeChild : 'component'}
      </button>
    </li>
  );
  ...
  return <ul className="Tree">{list}</ul>;
};
  ...
  return (
    <li className="Tree__node TreeNode">
      <div className="TreeNode__name">
        {node.component}
        <button className="AddButton AddButton--danger" onClick={handleDelete}>
          ⨯
        </button>
        <button className="AddButton AddButton--dull" onClick={handleUp}>
          ↑
        </button>
        <button className="AddButton AddButton--dull" onClick={handleDown}>
          ↓
        </button>
      </div>
      <div>
