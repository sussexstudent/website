...
const StatusBadge: React.FC<{ status: string }> = ({ status }) => {
  const color = `${status == 'done' ? COLORS.BRAND_GREEN : COLORS.BRAND_YELLOW}`;
  return (
    <div className={status} css={{
      ...
      }}></span>{status.replace('_',' ')}</div>
  )
}

export const Pledge: StreamFieldBlock<{ title: string, body: string, status: string, image: FalmerImage }> = ({
  ...
  return (
    <div className="Trail Trail__row--11" css={{marginBottom: '5%', flexDirection: reverse}}>
      <div css={itemClass}>
