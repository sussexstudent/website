...
interface IProps {
  page: any;
  items: Array<any>;
  renderItem?(props: {children: any, key: string, block: StreamFieldBlock<any>, page: Page<any>}): any;
}

class StreamField extends React.Component<IProps> {
  render() {
    const {
      items, page, renderItem
    } = this.props;
    return items.map(item => {
      ...
      if (Component) {
        if (renderItem) {
          return renderItem({
            children: <Component page={page} block={item.value} key={item.key} />,
            block: item.value,
            key: item.key,
            page: page,
          });
