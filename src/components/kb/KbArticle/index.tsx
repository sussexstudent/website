...
function KbArticle(props: IProps) {
  ...
  return (
    <div>
      <BackBar color="blue" to={`/help/${article.topic.section.slug}`}>{article.topic.section.title}</BackBar>
...
export default compose<IProps, OwnProps>(
  graphql<Result, OwnProps>(KbArticleQuery, {
    options: props => ({
      variables: {
        articlePath: [props.match.params.sectionSlug, props.match.params.topicSlug, props.match.params.articleSlug].join('/')
      }
    })
  }),
  apolloHandler()
