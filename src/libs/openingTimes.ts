import {getDay, addDays, setMinutes, setHours, isWithinInterval, isBefore} from "date-fns";
...
function generateDateForDay(day: number, time: string) {
  ...
  return setMinutes(setHours(addDays(now, daysFromNow), parseInt(hours, 10)), parseInt(minutes, 10));
}

function getDaysFromDays(days: string): number[] {
  return flatten(days.split(',').map(daysRange => {
...
const generateIntervalForDay = (startTime: string, endTime: string) => (day: number) => {
...
export function parseOpeningTimesToIntervals(openingTimes: string[]): OpeningTimeInterval[] {
